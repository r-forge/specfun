...
\Abstract{
  ...
  \to -\infty$ when $p\to 0$, \code{qnorm()} using AS~241 has been very
  inaccurate in the very extreme tails.

  We have derived asymptotic formulas for that case, using recursive
  plug-in to the asymptotic formula for $\Phi(x)$ (which \code{qnorm()}
  should invert).

  Using these formulas for order $k=0, 1, \dots, 5$, for six different regions (adjacent intervals)
  allows to provide fully accurate \code{qnorm()} computations also on the
  log-scale.
  Pure \R{} implementations of these are provided in our \R{} package \CRANpkg{DPQ},
  functions \code{qnormAsymp()} and \code{qnormR()} and have also been
  prepared to be added to (the C code in \file{Rmathlib} in) the next
  version of \R's \code{qnorm()}.
}
...
a (pure \R{} implementation) also to be used in
