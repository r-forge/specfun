...
\title{Asymptotic Tail Formulas For Gaussian Quantiles for R}
...
\Plaintitle{Asymptotic Tail Formulas For Gaussian Quantiles for R} % without formatting
% \Shorttitle{}
%
%\VignetteIndexEntry{Asymptotic Tail Formulas For Gaussian Quantiles for R}% << shown on CRAN page!
%\VignetteDepends{DPQ}
...
\newtheorem*{AS}{A.\ \& S.}% for formulas cited from Abramowitz & Stegn
...
Gaussian or normal quantiles have been made available in \R\ from the very
beginning.

Ross Ihaka (one of the two fathers of "R") wrote the first
version, in R's svn repos , rev 574, dated Jan.\ 14, 1998 % ~/R/D/r-devel/R/src/nmath/qnorm.c.~r574~
...
specifically, when
$p' \in (10^{-300}, \epsilon_c ]$, where the computer epsilon,
$\epsilon_c$ (= \code{DBL\_EPSILON} in C' \code{math} library $=$ \R's \code{.Machine$double.eps} %$
is nowadays always equal to $2^{-52} = 2.220446..\cdot 10^{-16}$.

This first algorithm, e.g. in  \R~1.0.0, Feb.29, 2000, as AS~111
e.g., Jan.18 2000, % ~/R/D/r-devel/R/src/nmath/qnorm.c.~r7639~
the svn rev~7639 version of  R/src/nmath/qnorm.c had
...
Already a few months after releasing R 1.0.0, namely on June 6, svn r9464,
%% the change is from rev 9363 to 9464
I had switched \code{qnorm()} to use the more recent and accurate  AS~241
...
\end{verbatim}
which, also by Wichura, from 1988, see \cite{AS241:Wichura1988}.
%--> ./qnorm-litt.bib    Wichura, M. J. (1988) Algorithm AS 241: ....

Note the promise of up to 16 digits precision, i.e., about
the usual IEEE 52-bit double precision ($2^{-52} \approx 2.22 \cdot 10^{-16}$).
...
The ``outermost'' minimax rational approximation to $-\Phi^{-1}(p)$ they use is in the interval
$r \in (5, 27] \Longleftrightarrow r^2 \in (25, 729]$, or equivalently,
\begin{align}
  p \in \left[e^{-729}, e^{-25}\right) \approx \left[2.51 10^{-317}, 1.389 10^{-11} \right).
\end{align}

At first, the above seems sufficient, since indeed, $10^{-317}$ is already
a ``de-normalized'' number in double precision arithmetic.
...
that reason we had introduced the option \code{log.p = TRUE} for all our distribution
...
This fact and my subsequent research finding accurate approximations in these
outermost tails is the goal of this paper.

\subsection[DPQ's qnormR()]{\pkg{DPQ}'s \code{qnormR()} documenting history}

Note that in our \pkg{DPQ}, we do provide pure R code implementations of
 \R's \code{qnorm()} in function \code{qnormR()} which has
(almost\footnote{`mu' $\neq$ `mean'}) the same arguments
\code{p, mu = 0, sd = 1, lower.tail = TRUE, log.p = FALSE} as \R's \code{qnorm()}
...
In order to compare versions of \code{qnorm()} approximations with their
...
pkg \CRANpkg{Rmpfr} with its own very accurate \code{pnorm()},
but we are not providing the evidence here.

With this assumption, the error of \code{qnorm()} is the deviation from the identity
$\Phi^{-1}(\Phi(x)) \equiv x$.
...
\code{log.p = TRUE} in both \code{pnorm()} and \code{qnorm()}, and we want
...
if(getRversion() <= "4.0.5") { # our qnormR(.., version="4.0.x")
    ## gives identical values as qnorm(..) then :
    stopifnot(identical(qnrm, qnrm405))
} else if(getRversion() < "4.3") # our qnormR(*, version="2020-10-17") matches:
    stopifnot(identical(qnrm, qnrm410))
@
...
\label{fig:qnorm405}
\end{figure}
looks good up to about $10^{12}$, i.e., \code{qnorm()} coinciding with the
true $x$ \code{qs}, but beyond $10^{12}$ diverging for larger $s$,
and for even larger $s$ showing
complete loss of accuracy as \code{qnorm(-s, *)} converges (to
98340296.60657 after about $s=10^{40}$), even though the true function
...
loss for extreme tail \code{qnorm()} was to use the above $\sqrt{2s}$
...
= \log p = -664991$ or $r = \sqrt{-\log p} = 815.470$ (with $r$ defined in (\ref{r.def})), and for convenience (round number), using the cutoff $r \ge 816$ in \code{qnorm()}, i.e., basically
\begin{Schunk}
\begin{Sinput}
...
Indeed, \code{qnorm()} was now ``first order accurate'' even in the extreme tails, and in a plot
...
In \R, the function \code{qnorm()} and notably the underlying C API
function \code{qnorm()}\footnote{\R's C API \code{qnorm()} is an alias for
  \code{qnorm5()} in the source file \file{<R>/src/nmath/qnorm.c}}
...
may naturally be desirable that \code{qnorm()} computes its values to the
...
$\Phi(.)$, i.e., \R's \code{pnorm()}, has been known to many more terms,
...
\begin{align}
  \label{eq:AS_26.2.12}
  1 - \Phi(x) = \frac{\phi(x)}{x} \cdot\left(1 - \frac 1{x^2} +
    \frac{1\cdot 3}{x^4} + \cdots + \frac{(-1)^n\; 1\cdot
      3\cdots(2n-1)}{x^{2n}}\right) \ + \ R_n,
\end{align}
\end{AS}
where the remainder term $R_n$ (it has an exact representation via an
...
\code{qnorm()}), and we would like to \emph{solve} for $x$, as in the
...
Fig.~\ref{fig:qnormAsym} below, where we show further steps,
...
\end{align*}
These $x_k(s)$ are provided as plain \R{} function \code{qnormAsymp()}, in
...
qnAsym <- sapply(setNames(k.s, nks), \(k) qnormAsymp(lp=lp, order = k))
...
Our (package \CRANpkg{DPQ}) \code{qnormR(... version = "2022-08")} now implements
a (pure \R{} implementation) also to be used in the next (yearly) version of R,
...
\paragraph{Optimal cut points / regions for different approximation orders $k$}
...
These were determined using function \code{p.qnormAsy2()} in appendix \ref{sec:p.qnormAsy},
for visualizing the
optimal region for switching from $k-1$ to $k$, for $k = 1,2,3,4,5$, with this code
...
@
To see how the final \code{qnormR()} is implemented, you can look at the \code{length-1} version
\code{qnormR1}\footnote{indeed, in the package source file \verb|DPQ/R/norm_f.R|,
  \code{qnormR()} is defined to correctly vectorize in its main arguments \code{p}, \code{mu},
...
\section[relErrV() (from sfsmisc)]{%
  Function \code{relErrV()} (package \pkg{sfsmisc})}\label{sec:relErrV}
...
\code{relErrV()} from (our own) CRAN package \CRANpkg{sfsmisc},
...
\section[p.qnormAsy2() for cutpoints]{%
  Function \code{p.qnormAsy2()} for showing optimal cutpoints}\label{sec:p.qnormAsy}

This function, currently also used in \pkg{DPQ}'s \code{example(qnormAsymp)}, was used by the author and
may be used for reproducibility to visualize the five ``cutpoint - regions'' to switch from approximation $x_{k-1}(r)$ to $x_k(r)$, for $k=1,\dots,5$ and  $r = \sqrt{s} = \sqrt{- \log p}$, using something like
<<call-p.qnormAsy2, eval=FALSE>>=
r0 <- c(27, 55, 109, 840, 36000, 6.4e8)
for(ir in 2:length(r0))
    p.qnormAsy2(r0[ir], k = 5 +2-ir)
@
...
\setkeys{Gin}{width=0.95\textwidth}% default 0.8
...
r0 <- c(27, 55, 109, 840, 36000, 6.4e8)
for(ir in 2:length(r0))
    p.qnormAsy2(r0[ir], k = 5 +2-ir, verbose=FALSE, cex.main = 1)
@
