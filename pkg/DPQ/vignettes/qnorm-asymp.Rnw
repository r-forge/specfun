...
(with $r$ defined in (\ref{r.def})), and for convenience (round number), using the cutoff $r \ge 816$ in \fct{qnorm}, i.e., basically
\begin{Schunk}
\begin{Sinput}
...
$\epsilon_c = 2^{-52} \approx 2.22 \cdot 10^{-16}$, indeed the asymptotic $x_5(s)$ see (\ref{x5sq}) may be seen to be even better than algorithm AS~241 for the small region $s \in [22.8^2, 27^2]=[519.84, 729]$. %
...
<<relE43>>=
relE_qn <- relErrV(qs, qnrm43); version.txt <- "R > 4.2.x (after 2022)"
@
