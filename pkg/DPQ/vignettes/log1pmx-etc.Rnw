...
\begin{align}
  ...
  D_0(x,M) % = M \cdot [(t+1)(\log(1+t) - 1) + 1]
           &= \overbrace{M\cdot(1+t)}_{x} \log(1+t) - \overbrace{t \cdot M}_{x-M} =
...
\section{Appendix A: Accuracy of log1pmx(x) Computations}
...
l1r <- cbind(as.data.frame(l1r), y.naive = with(l1r, -(log(1+x)-x)))
c4 <- adjustcolor(4, 1/3)
lines(y.naive ~ x, data=l1r, col=c4, lwd=3, lty=2)
legend("left", legend=expression(- log1pmx(x), -(log(1+x)-x)),
       col=c(palette()[2],c4), lwd=c(1,3), lty=1:2, bty="n")
@% \) {{de-confusing emacs parser}}
...
l1r <- cbind(l1r, relE.naive = with(l1r, sfsmisc::relErrV(y, y.naive)))
plot(relE.naive ~ x, data=l1r, type="l", ylim = c(-1,1)*1e-6)
y2 <- 1e-8
rect(-.002, -y2, .002, y2, col=adjustcolor("gray",1/2), border="transparent")
zoomTo(15e-4, 9*y2, 13e-4, -y2)
plot(relE.naive ~ x, data=l1r, type="l", ylim = c(-1,1)*y2); abline(h=0,lty=3)
@
% y2 <- 1e-10
...
\section{Appendix B: Accuracy of $ p_1l_1(t)$ Computations}
%%
